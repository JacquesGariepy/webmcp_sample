...
function registerCatalogue() {
  ...
  tool({ name: "reading_list_mark_all_read", tier: "write", description: "Mark every unread item as read, optionally only those with a tag. Returns the count.", input: obj({ tag: { type: "string" } }), run: ({ tag }) => { const hit = state.items.filter(i => !i.read && (!tag || i.tag === tag)); for (const i of hit) markRead(i.id, true, "agent"); return { marked: hit.length }; } });
